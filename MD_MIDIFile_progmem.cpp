...
void MD_MIDIFile::begin(byte *sffs)
{
  current_point = 0;
  _sd = sffs;
}
...
int MD_MIDIFile::load(byte *progmem_array) 
...
// Return one of the E_* error codes
{
  ...
  // header chunk = "MThd" + <header_length:4> + <format:2> + <num_tracks:2> + <time_division:2>
  {
    ...
    for(int i = 0 ; i < MTHD_HDR_SIZE ; i ++) h[i] = pgm_read_byte(&progmem_array[i]);
    current_point = MTHD_HDR_SIZE;
  ...
  // read header size
  dat32 = readMultiByte(_sd + current_point, MB_LONG);
  current_point += MB_LONG;
  //Serial.print("dat32=");
  ...
  // read file type
  dat16 = readMultiByte(_sd + current_point, MB_WORD);
  current_point += MB_WORD;
  if ((dat16 != 0) && (dat16 != 1))
  {
    //_fd.close();
    return(E_FORMAT);
