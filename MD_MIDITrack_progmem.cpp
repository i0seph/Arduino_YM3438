...
// track_event = <time:v> + [<midi_event> | <meta_event> | <sysex_event>]
{
  ...
  mf->current_point = _startOffset+_currOffset;
  ...
  cnt = readVarLen(mf->_sd + mf->current_point, &deltaT);

  mf->current_point += cnt;
  ...
  _currOffset = mf->current_point - _startOffset;
...
// process the event from the physical file
{
  ...
  ch[0] = pgm_read_byte(mf->_sd + mf->current_point);
  mf->current_point++;
  eType = ch[0];
...
// ---------------------------- SYSEX
  ...
  case 0xf7:  // sysex_event = 0xF7 + <len:1> + <data_bytes> + 0xF7 
  {
    ...
    sev.track = _trackId;
    cnt = readVarLen(mf->_sd + mf->current_point, &mLen);
    mf->current_point += cnt;
    sev.size = mLen;
    ...
    for (uint16_t i=index; i<minLen; ++i) {
      ch[0] = pgm_read_byte(mf->_sd + mf->current_point + i);
      mf->current_point++;
      sev.data[i] = ch[0];
    }
    if (sev.size>minLen)
      mf->current_point += sev.size-minLen;
...
// ---------------------------- META
  case 0xff:  // meta_event = 0xFF + <meta_type:1> + <length:v> + <event_data_bytes>
  {
    ...
    switch (eType)
    {
      ...
      {
        uint32_t value = readMultiByte(mf->_sd + mf->current_point, MB_TRYTE);
        mf->current_point += MB_TRYTE;
      ...
      {
        uint16_t x = readMultiByte(mf->_sd + mf->current_point, MB_WORD);
        mf->current_point += MB_WORD;
      ...
      mev.data[0] = readMultiByte(mf->_sd + mf->current_point, MB_BYTE);
      mf->current_point += MB_BYTE;
...
// return -1 if success, 0 if malformed header, 1 if next track past end of file
{
  ...
  // track_chunk = "MTrk" + <length:4> + <track_event> [+ <track_event> ...]
  {
    ...
    for(int i = 0 ; i < MTRK_HDR_SIZE ; i ++) h[i] = pgm_read_byte(mf->_sd + mf->current_point + i);
    //mf->_fd.readBytes(h, MTRK_HDR_SIZE);
    mf->current_point += MTRK_HDR_SIZE;
    h[MTRK_HDR_SIZE] = '\0';
  ...
  dat32 = readMultiByte(mf->_sd + mf->current_point, MB_LONG);
  mf->current_point += MB_LONG;
  _length = dat32;

  // save where we are in the file as this is the start of offset for this track
  _startOffset = mf->current_point;
  _currOffset = 0;

  // Advance the file pointer to the start of the next track;
  /*
  if (!mf->_sd[_startOffset+_length]){
    return(1);
    //Serial.println("error!");
  }
  */
  mf->current_point += _length;
  return(-1);
}
